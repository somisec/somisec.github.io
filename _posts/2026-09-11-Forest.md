...
We can try to do different attacks here like if a user has the same password as username. But we are going to do a kerberoasting. Normaly we need credentials to authenticate with from the start but there is a option for a account to have the property. "Do not require Kerberos preauthentication” or UF_DONT_REQUIRE_PREAUTH set to true.
...
Now lets just secretsdump to get the Administrator hash.
