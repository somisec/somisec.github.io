...
We can try to do different attacks here like if a user has the same password as username. But we are going to do a kerberoasting. Normaly we need credentials to authenticate with but there is a option for a account to have the property. "Do not require Kerberos preauthentication” or UF_DONT_REQUIRE_PREAUTH set to true. I start with the "normal usernames" first. If none work I would test the rest.
...
After that we can WriteDacl over HTB.LOCAL. We are gonna give ourselfs the rights 'DCSync', this is in order to be able to run secretsdump.
